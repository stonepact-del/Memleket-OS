...
const listingSchema = z.strictObject({ id, category:z.enum(['Vasıta','Emlak','Elektronik','Ev & Yaşam']), title:text, price:money, marketValue:money, condition:metric, seller:text, favorite:z.boolean(), status:z.enum(['active','sold','expired']), details:text, hiddenIssue:text.optional() });
...
const applicationSchema = z.strictObject({ id, jobId:id, state:z.enum(['submitted','viewed','rejection','no response','interview','later stage','offer','accepted','withdrawn']), updatedAt:timestamp });
...
export const saveSchema: z.ZodType<State> = z.strictObject({
  schemaVersion:z.literal(2), simulationVersion:z.string().min(1), worldSeed:integer.nonnegative(), characterSeed:integer.nonnegative(), rngState:integer.nonnegative(), id, createdAt:timestamp, updatedAt:timestamp, now:timestamp, speed:z.union([z.literal(0),z.literal(1),z.literal(4),z.literal(12)]),
  ...
  vehicles:z.array(z.strictObject({ listingId:id, title:text, value:money, condition:metric })), messages:z.array(messageSchema), posts:z.array(z.strictObject({ id, npcId:id, at:timestamp, text, likes:integer.nonnegative(), liked:z.boolean() })), mails:z.array(z.strictObject({ id, at:timestamp, sender:text, subject:text, body:text, read:z.boolean() })),
  holdings:z.record(z.string(),z.strictObject({ quantity:integer.nonnegative(), cost:money })), notes:text, settings:z.strictObject({ sound:z.boolean(), reducedMotion:z.boolean(), largeText:z.boolean() }), processedEventIds:z.array(id)
});
