...
export interface GameEvent { id:string; at:string; type:string; title:string; important:boolean; processed?:boolean; entityIds?:string[]; requiresInput?:boolean; details?:string }
...
export interface Application { id:string; jobId:string; state:'submitted'|'viewed'|'rejection'|'no response'|'interview'|'later stage'|'offer'|'accepted'|'declined'|'withdrawn'; updatedAt:string; interviewAt?:string; statusUnread:boolean }
...
const eventSchema = z.strictObject({ id, at:timestamp, type:text, title:text, important:z.boolean(), processed:z.boolean().optional(), entityIds:z.array(id).optional(), requiresInput:z.boolean().optional(), details:text.optional() });
...
export const saveSchema: z.ZodType<State> = z.strictObject({
  ...
  companies:z.array(companySchema), jobs:z.array(jobSchema), applications:z.array(applicationSchema), employment:z.strictObject({ jobId:id, performance:metric, startedAt:timestamp, nextPayAt:timestamp }).optional(), listings:z.array(listingSchema),
